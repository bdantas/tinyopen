...
\* If you're on Tiny Core Linux, `tinyopen.tcz` extension is available and does not have the `tinyopen-setup` script because the extension takes care of the setup for you. If you really want to use the `tinyopen` directory as found here on github, put the directory in persistent storage (e.g., in `/home` or `/opt` depending on your setup) and run `/path/to/tinyopen-setup` at each boot (e.g., via a startup job in `~/.X.d/`). The reason the setup needs to be done at each boot is that TCL's root filesystem exists only in RAM therefore tweaks to the filesystem do not survive reboots.
